...
// Manter URLs padrão para compatibilidade (sandbox)
...
export async function obterToken(contratoId?: number, forceRefresh = false, retryCount = 0): Promise<string> {
  ...
  const isSandbox = contrato.IS_SANDBOX === 'S';
  ...
  tokenPromise = (async () => {
    try {
      ...
      // Fazer login na API Sankhya com as credenciais do contrato
...
export async function fazerRequisicaoAutenticada(fullUrl: string, method = 'POST', data = {}, retryCount = 0, contratoId?: number) {
  ...
  try {
    ...
    console.log(`🔑 [Requisição] Usando Bearer Token do contrato ${contratoId || 'padrão'}: ${bearerToken.substring(0, 30)}...`);
